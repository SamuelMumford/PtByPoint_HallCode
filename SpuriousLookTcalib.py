...
def main():
    ...
    #Read in the file
    fname = 'C:/Users/sammy/Downloads/9_12_20_cool2.txt'
    f2 = 'C:/Users/sammy/Downloads/9_12_20_cool2L.txt'
    ...
    if(DelOutliersTemp):
        ...
        runb, temb, currb = np.zeros(0), np.zeros(0), np.zeros(0)
        for j in range(len(runs)-1):
            ...
            if(currs[j] != currs[j+1] or j ==len(runs)-2):
                ...
                #of all data
                    ...
                    runb = np.append(runb, rb[startIndex:j])
                    temb = np.append(temb, rb[startIndex:j])
                    currb = np.append(currb, cb[startIndex:j])
                    startIndex = j+1
                    index += 1
        BsTemp = currb*.85/5
        runsTemp = runb
    ...
    tSl = np.abs(tdmaker(BsTemp, tempsTemp))
    plt.plot(runsTemp, tSl)
    plt.ylim(0, .002)
    plt.show()
    limmy = 1
    crit = (tSl < limmy)
    ...
    plt.plot(rsFin, tsFin - wrapT(RandB, *pSub))
    plt.show()
    ...
    order = np.linspace(0, len(Blist) + 1, len(Blist))
    power = 1
